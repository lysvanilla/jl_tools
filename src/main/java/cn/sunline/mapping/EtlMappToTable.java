...
public class EtlMappToTable {
     ...
     */
    public static void etlMappToTableMain(String filePath) {
        ...
        if (FileUtil.isDirectory(filePath)) {
            ...
            for (File file : FileUtil.ls(filePath)) {
                ...
                if (fileName.endsWith(".xlsx") && !fileName.startsWith("~") && !fileName.endsWith("0_封面.xlsx")
                    ...
                    // 处理单个文件，获取表结构信息
                    tableStructureListAll.addAll(etlMappToTable(file.getAbsolutePath()));
                }else{
            ...
            // 处理单个文件，获取表结构信息
            tableStructureListAll.addAll(etlMappToTable(filePath));
        }
     ...
     * @return 转换后的表结构信息列表。
     */
    public static List<TableStructure> etlMappToTable(String filePath) {
        log.debug("开始从文件 {} 中读取 ETL 映射信息。", filePath);
        // 从文件中读取 ETL 映射信息
        List<EtlMapp> etlMappList = readEtlMappExcel(filePath);
        log.debug("从文件 {} 中读取到 {} 条 ETL 映射信息。", filePath, etlMappList.size());
    ...
    public static void etlMappToTable(List<TableStructure> tableStructureList) {
        ...
        // 将表字段信息写入 Excel 文件
        writeTableExcel(tableFieldInfoListAll, TPL_PATH, outputFilePath);
    }
